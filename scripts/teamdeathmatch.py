...
class TDMConnection(TeamConnection):
    ...
    def on_load(self):
        super(TDMConnection, self).on_load()
    ...
    def on_player_kill(self, player):
        ...
        if reactor.seconds() - self.last_kill > self.multikill_time:
            self.multikill = 0
        self.multikill += 1
        ...
        if spree_index > old_spree_index:
            self.server.send_chat(self.spree_names[spree_index-1] % killer.name)

        self.last_kill = reactor.seconds()

        # give myself xp
        self.give_kill_xp(player)

        # give anyone that assisted me in killing assist xp
        for assist, t in player.assists.iteritems():
            if assist is None:
                continue
            if assist == self:
                continue
            if reactor.seconds() - t < self.tag_duration:
...
class TDMServer(TeamServer):
    ...
    def on_load(self):
        ...
        reactor.callLater(5.0, self.start_round_delayed)
    ...
    def get_scores(self):
        scores = []
        for name, t in self.teams.iteritems():
